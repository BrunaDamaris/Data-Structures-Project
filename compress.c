...
unsigned char print_compress(tree *hufftree, unsigned char compbyte, unsigned int *indexgeral,unsigned int *indexcurrent,unsigned char byteprevious, FILE *file_out, unsigned int height)
{
	//printf("compress_7\n");
	...
	while(!feof(file_in))
	{
		if(fscanf(file_in,"%c",&filebyte) != EOF)
		{
			...
			if(filebyte != 0xff && filebyte != 0x0a)
			{
				...
				byte = print_compress(hufftree,hashtree->trees[key]->path_bits,index_geral_freq,index_current_freq,byte,file_out,hashtree->trees[key]->height);
